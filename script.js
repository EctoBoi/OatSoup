...
function render() {
    ...
    if (showHit) {
        ...
        if (hit === 1) {
            ...
            ctx.beginPath();
            ctx.arc(lastNotePos[0], lastNotePos[1], radius / size + 4, 0, 2 * Math.PI)
            ctx.fillStyle = 'black'
            ctx.fill()

            ctx.beginPath();
            ctx.arc(lastNotePos[0], lastNotePos[1], radius / size + 2, 0, 2 * Math.PI)
            ctx.fillStyle = 'white'
            ctx.fill()
        }
    ...
    if (noteDirections[currentNote] === undefined)
        ctx.strokeStyle = 'black'
    ctx.stroke()
    ...
    if (noteDirections[currentNote + 1] === 0)
        ctx.fillStyle = 'blue'
    if (noteDirections[currentNote + 1] === 1)
        ctx.fillStyle = 'red'
    if (noteDirections[currentNote + 1] === 2)
        ctx.fillStyle = 'green'
    if (noteDirections[currentNote + 1] === undefined)
        ctx.fillStyle = 'black'
    ctx.fill()
    ...
    if (showMessage) {
        ...
        ctx.fillText(message, (canvas.width / 2) - (ctx.measureText(message).width / 2), 35)
    }
...
function playSound() {
    ...
    gain.gain.value = .1; // from 0 to 1, 1 full volume, 0 is muted
    gain.gain.exponentialRampToValueAtTime(
        0.001, context.currentTime + 0.04
    )
...
function checkInput() {
    if (lastKey[1] - noteTime > -40 && lastKey[1] - noteTime < 40) {
        if (noteDirections[currentNote - 1] === 0) {
            if (lastKey[0] === 'ArrowRight')
                noiceHit()
            else
                miss()
        }
        else if (noteDirections[currentNote - 1] === 1) {
            if (lastKey[0] === 'ArrowLeft')
                noiceHit()
            else
                miss()
        }
...
function moveTimer() {
    if (activeStepNum < activeMoveSteps) {
        ...
        if (noteDirections[currentNote] === 2) {
            notePosX = notePos[0] + (radius / (activeMoveSteps / (activeMoveSteps - activeStepNum))) * Math.cos(activeStep)
            notePosY = notePos[1] + (radius / (activeMoveSteps / (activeMoveSteps - activeStepNum))) * Math.sin(activeStep)
        }

        activePos = [notePosX, notePosY]
        if (activeStepNum % 6 === 0)
            notePath.push(activePos)

        currentActiveTimer = setTimeout(moveTimer, (noteLengths[currentNote] / activeMoveSteps))
    } else {
...
function tetrisStart() {
    ...
    noteLengths = [300, 600, 300, 300, 600, 300,
        300, 600, 300, 300, 600, 300,
        300, 900, 300, 600,
        600, 600, 600,
        1200, 600, 300, 600, 300,
        300, 900, 300, 600, 300,
        300, 600, 300, 300, 600,
        600, 600, 600]
    noteDirections = [0, 1, 1, 0, 2, 0,
        1, 2, 1, 1, 0, 0,
        1, 2, 1, 2,
        0, 0, 2,
        1, 1, 1, 2, 0,
        ...
        0, 0, 1]
    restart()
    noteHz = [1319, 988, 1047, 1175, 1047, 988,
        880, 880, 1047, 1319, 1175, 1047,
        988, 1047, 1175, 1319,
        1047, 880, 880,
        1175, 1397, 1760, 1568, 1397,
        1319, 1047, 1319, 1175, 1047,
        988, 988, 1047, 1175, 1319,
        1047, 880, 880]
...
function randomStart() {
    ...
    for (let i = 0; i < noteCount; i++) {
        ...
        if (i === 0)
            randomNoteDirections.push(getRandomInt(0, 1))
        else
            randomNoteDirections.push(getRandomInt(0, 2))
    }
...
function seededStart() {
    ...
    if (regex.test(seed)) {
        ...
        for (let i = 0; i < seed.length; i++) {
            ...
            //lengths
            const lengthSets = [[300, 350], [350, 400], [350, 500], [450, 300], [500, 450],
            [300, 650], [350, 700], [350, 800], [450, 600], [500, 850],
            [650, 600], [700, 650], [800, 650], [600, 750], [850, 800],
            [650, 300], [700, 350], [800, 350], [600, 450], [850, 500],
            [600, 650], [650, 700], [650, 800], [750, 600], [800, 850], [700, 850]]

            seededNoteLengths.push(lengthSets[indexedCharCode][0] + difficultyMod)
            seededNoteLengths.push(lengthSets[indexedCharCode][1] + difficultyMod)

            //directions
            const lastNum = parseInt((charCode + "").slice(-1))
            const directionSets = [[0, 0], [0, 1], [1, 0], [2, 2], [0, 2], [2, 0], [1, 0], [1, 1], [1, 2], [2, 1]]

            if (i === 0 && directionSets[lastNum][0] === 2)
                seededNoteDirections.push(0)//start with clockwise if crossover
            else
                seededNoteDirections.push(directionSets[lastNum][0])
